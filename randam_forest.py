...
#降順で並び替え・インデックスでスライス・特徴量取り出し。
#★注意★：グラフ描写の都合で、データフレーム内は昇順になっている。
dftop8_importance = pd.DataFrame({'features':df_train.columns,'importance':clf.feature_importances_}).sort_values(by='importance',ascending=True)


# %%
####################################################################
#上位８つの特徴量から一つずつ抜く。→グラフ出力、精度の結果をエクセルに出力
####################################################################

#top8_dfの特徴量のリスト

top8_list = list(top8_df.drop(['Plankton sp.'],axis=1).columns)
random_state = 1000#乱数は1000から毎回１増やす。
scores_list = []

for ft in top8_list:
    #特徴量落としたデータを作成、学習とテストを分割
    drop_df = top8_df.drop([ft],axis=1)#列１つドロップDFを作成
    drop_label, drop_features = sepLabelData(drop_df)#ラベルと特徴量分割
    df_train, df_test, label_train, label_test = train_test_split(drop_features, drop_label, test_size=0.25, random_state = random_state)
    
    #学習
    clf = RandomForestClassifier(n_estimators=150, random_state = random_state) #(決定木の数,ランダム固定(任意の数))
    clf.fit(df_train, label_train) #fit()メソッドでモデルに学習(学習データ)
    # scores_list.append(clf.score(df_test, label_test))#精度をリストに追加

    #特徴量の重要度のグラフを作成、保存、重要度を
    dfleft_importance = pd.DataFrame({'features':df_train.columns,'importance':clf.feature_importances_}).sort_values(by='importance',ascending=True)
    file_name = r'C:\Users\●●●●●●●●●●'+'\drop_'+ ft +'png'
    savePlotImportance(dfleft_importance,file_name)
    dfleft_importance.to_excel(r'C:\Users\●●●●●●●●●●'+'\drop_'+ ft +'.xlsx')
...
pd.DataFrame({'削除した特徴量':top8_list,'精度':scores_list}).to_excel(r"C:\Users\sato_t\Desktop\temp\研究開発pythonコード\上位8特徴量1つずつドロップ.xlsx")
